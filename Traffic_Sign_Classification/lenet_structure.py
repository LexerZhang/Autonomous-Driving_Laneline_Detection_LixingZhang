...
def lenet_tf(X_input, keep_prob, drop_out=True):
    ...
    w_dic = {  # The dictionary for all weights variables & sizes
        "conv1": tf.Variable(tf.compat.v1.truncated_normal((5, 5, 1, 6), mean=mu, stddev=sigma), name="w_conv1"),
        "pool1": [1, 2, 2, 1],
        "conv2": tf.Variable(tf.compat.v1.truncated_normal((5, 5, 6, 16), mean=mu, stddev=sigma), name="w_conv2"),
        "pool2": [1, 2, 2, 1],
        "linear1": tf.Variable(tf.compat.v1.truncated_normal((400, 120), mean=mu, stddev=sigma), name="w1"),
        "linear2": tf.Variable(tf.compat.v1.truncated_normal((120, 84), mean=mu, stddev=sigma), name="w2"),
        "linear3": tf.Variable(tf.compat.v1.truncated_normal((84, 43), mean=mu, stddev=sigma), name="w3")
    }
    ...
    flatten_output = tf.compat.v1.layers.flatten(pool2_output)
